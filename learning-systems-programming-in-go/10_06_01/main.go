...
func (cf *CloudFileSystem) Getattr(path string,
	stat *fuse.Stat_t, fh uint64) (errc int) {
	if path == "/" {
...
func (cf *CloudFileSystem) Read(path string,
	buff []byte, ofst int64, fh uint64) (n int) {
	name := strings.TrimLeft(path, "/")
	ctx := context.Background()
...
func (cf *CloudFileSystem) Open(path string, flags int) (int, uint64) {
	return 0, ^uint64(0)
}
